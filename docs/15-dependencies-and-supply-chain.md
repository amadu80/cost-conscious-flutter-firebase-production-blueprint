...
Verify every AI-suggested dependency directly in the authoritative registry and upstream repository before installation. Check exact spelling, publisher identity, release history, signatures or provenance where available, license, maintenance, known vulnerabilities, transitive dependencies, and requested permissions. A build failure is not a reason to install a similarly named package suggested by a model.
